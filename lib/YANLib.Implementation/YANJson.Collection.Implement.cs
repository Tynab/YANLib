...
internal static partial class YANJson
{
    ...
    internal static IEnumerable<T?>? DeserializesFromBytesImplement<T>(this IEnumerable<byte[]?>? input, JsonSerializerOptions? options = null) => input.IsNullEmptyImplement()
        ? default
        : input.GetCountImplement() < 1_000
        ? options.IsNullImplement() ? input.Select(x => x.DeserializeFromBytesImplement<T>()) : input.Select(x => x.DeserializeFromBytesImplement<T>(options))
        : options.IsNullImplement() ? input.AsParallel().Select(x => x.DeserializeFromBytesImplement<T>()) : input.AsParallel().Select(x => x.DeserializeFromBytesImplement<T>(options));
}
