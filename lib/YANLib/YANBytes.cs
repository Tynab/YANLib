...
public static partial class YANBytes
{
    ...
    public static byte[]? ToByteArray(this object? input) => input.IsNull() ? default : UTF8.GetBytes(input.StandardSerialize() ?? string.Empty);
    ...
    public static IEnumerable<byte[]?>? ToByteArrays(this IEnumerable<object?>? input) => input.IsNullOEmpty() ? default : input.Select(x => x.ToByteArray());
    ...
    /// <returns>An enumerable collection of byte arrays representing the UTF-8 encoded serializations of the input objects, or <see langword="null"/> if the input collection is <see langword="null"/> or empty.</returns>
    public static IEnumerable<byte[]?>? ToByteArrays(this ICollection<object?>? input) => input.IsNullOEmpty() ? default : input.Select(x => x.ToByteArray());
    ...
    public static IEnumerable<byte[]?>? ToByteArrays(params object?[]? input) => input.IsNullOEmpty() ? default : input.Select(x => x.ToByteArray());
    ...
    public static T? FromByteArray<T>(this byte[]? input) => input.IsNull() ? default : UTF8.GetString(input).Deserialize<T>();
    ...
    /// </returns>
    public static IEnumerable<T?>? FromByteArrays<T>(this IEnumerable<byte[]?>? input) => input.IsNullOEmpty() ? default : input.Select(x => x.FromByteArray<T>());
    ...
    /// </summary>
    /// <typeparam name="T">The type of objects to which the byte arrays are to be deserialized.</typeparam>
    /// <param name="input">The ICollection of byte arrays to be deserialized. Can be <see langword="null"/>.</param>
    /// <returns>
    /// An enumerable collection of objects of type <typeparamref name="T"/> representing the deserialized results from the byte arrays, or <see langword="null"/> if the input collection is <see langword="null"/> or empty.
    /// </returns>
    public static IEnumerable<T?>? FromByteArrays<T>(this ICollection<byte[]?>? input) => input.IsNullOEmpty() ? default : input.Select(x => x.FromByteArray<T>());
    ...
    public static IEnumerable<T?>? FromByteArrays<T>(params byte[]?[]? input) => input.IsNullOEmpty() ? default : input.Select(x => x.FromByteArray<T>());
}
