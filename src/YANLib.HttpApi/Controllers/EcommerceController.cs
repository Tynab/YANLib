...
public sealed class EcommerceController(ILogger<EcommerceController> logger, IEcommerceService service) : YANLibController
{
    ...
    [SwaggerOperation(Summary = "Lấy access token của API login của trang Ecommerce", Description = "Xác thực thông tin đăng nhập và trả về access token để truy cập các API của Ecommerce")]
    ...
    [SwaggerOperation(Summary = "Lấy refresh token của API refresh của trang Ecommerce", Description = "Sử dụng access token hiện tại để lấy refresh token mới")]
    [ProducesResponseType(typeof(object), Status200OK)]
    [ProducesResponseType(Status400BadRequest)]
    [ProducesResponseType(Status401Unauthorized)]
