...
{
    public static IServiceCollection AddEs(
        ...
        try
        {
            ...
            if (indexMappings.IsNotNull())
            {
                foreach (var mapping in indexMappings)
                {
                    ...
                    var indexName = (mapping.Value ?? $"{indexType.Namespace?.Split('.')[1]}_{indexType.Name
                    ...
                    _ = typeof(EsConfiguration).GetMethod(nameof(ConfigureDefaultMappingGeneric), NonPublic | Static)?.MakeGenericMethod(indexType)?.Invoke(null, [settings, indexName]);

                    if (!client.Indices.Exists(indexName).Exists)
                    {
                        _ = typeof(EsConfiguration).GetMethod(nameof(CreateIndexGeneric), NonPublic | Static)?.MakeGenericMethod(indexType)?.Invoke(null, [client, indexName]);
                    }
                }
            }
